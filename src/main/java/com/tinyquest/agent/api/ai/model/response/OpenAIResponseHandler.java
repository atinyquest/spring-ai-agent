...
import com.tinyquest.agent.api.ai.model.response.CompletionResponse.Annotation;
import com.tinyquest.agent.api.ai.model.response.CompletionResponse.FunctionCall;
import com.tinyquest.agent.api.ai.model.response.CompletionResponse.ToolCall;
        ...
        try {
            ...
            if (hasToolCalls) {
                for (JsonNode callNode : messageNode.path("tool_calls")) {
                    ...
                    String result = registry.executeFunction(call.function().name(), call.function().arguments().toString());
            ...
            if (hasFunctionCall) {
                ...
                String result = registry.executeFunction(func.name(), func.arguments().toString());
                followupMessages.add(Map.of(
                        "role", "function",
                        "name", func.name(),
                        "content", result
